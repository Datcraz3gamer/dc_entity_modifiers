...
execute store success score @s em_Success run execute as @e[type=!#nomodify,limit=1,sort=nearest,distance=..1] run data merge entity @s {Invulnerable:0b}
...
tag @e[tag=invuln,tag=em_tag,type=!#nomodify,limit=1,sort=nearest,distance=..1] remove invuln